...
impl Memory {
    ...
    pub fn load_half_word(
        ...
        let byte0 = builder.read(
            sorts.sid_byte,
            memory,
            vaddr,
            Some("load byte 0".to_string()),
        );
        let addr1 = builder.add(
            sorts.sid_machine_word,
            vaddr,
            consts.nid_machine_word_1,
            None,
        );
        let byte1 = builder.read(
            sorts.sid_byte,
            memory,
            addr1,
            Some("load byte 1".to_string()),
        );
    ...
    pub fn load_word(
        ...
        let addr2 = builder.add(
            sorts.sid_machine_word,
            vaddr,
            consts.nid_machine_word_2,
            None,
        );
        let byte2 = builder.read(sorts.sid_byte, memory, addr2, None);
        let addr3 = builder.add(
            sorts.sid_machine_word,
            vaddr,
            consts.nid_machine_word_3,
            None,
        );
    ...
    pub fn load_double_word(
        ...
        let byte3 = builder.read(sorts.sid_byte, memory, addr3, None);
        let addr4 = builder.add(
            sorts.sid_machine_word,
            vaddr,
            consts.nid_machine_word_4,
            None,
        );
        let byte4 = builder.read(sorts.sid_byte, memory, addr4, None);
        let addr5 = builder.add(
            sorts.sid_machine_word,
            vaddr,
            consts.nid_machine_word_5,
            None,
        );
        let byte5 = builder.read(sorts.sid_byte, memory, addr5, None);
        let addr6 = builder.add(
            sorts.sid_machine_word,
            vaddr,
            consts.nid_machine_word_6,
            None,
        );
        let byte6 = builder.read(sorts.sid_byte, memory, addr6, None);
        let addr7 = builder.add(
            sorts.sid_machine_word,
            vaddr,
            consts.nid_machine_word_7,
            None,
        );
        ...
        builder.concat(
            sorts.sid_double_word,
            w1,
            w0,
            Some("double-word (LE)".to_string()),
        )
    }
    ...
    pub fn load_value(
        ...
        match load_width {
            1 => {
                let byte =
                    builder.read(sorts.sid_byte, memory, vaddr, Some("load byte".to_string()));
                ...
                if sign_extend {
                    builder.sext(
                        ...
                        byte,
                        ext_bits,
                        Some("sign-extend byte".to_string()),
                    )
                } else {
                    builder.uext(
                        sorts.sid_machine_word,
                        byte,
                        ext_bits,
                        Some("zero-extend byte".to_string()),
                    )
                }
            }
            2 => {
                ...
                if sign_extend {
                    builder.sext(
                        ...
                        hw,
                        ext_bits,
                        Some("sign-extend half-word".to_string()),
                    )
                } else {
                    builder.uext(
                        sorts.sid_machine_word,
                        hw,
                        ext_bits,
                        Some("zero-extend half-word".to_string()),
                    )
                }
            }
            4 => {
                ...
                if word_bits > 32 {
                    ...
                    if sign_extend {
                        builder.sext(
                            ...
                            w,
                            ext_bits,
                            Some("sign-extend word".to_string()),
                        )
                    } else {
                        builder.uext(
                            sorts.sid_machine_word,
                            w,
                            ext_bits,
                            Some("zero-extend word".to_string()),
                        )
                    }
                } else {
                    w
            ...
            8 => Self::load_double_word(builder, sorts, consts, memory, vaddr),
    ...
    pub fn store_byte(
        ...
        let byte_val = builder.slice(
            sorts.sid_byte,
            value,
            7,
            0,
            Some("extract byte".to_string()),
        );
        builder.write(
            array_sid,
            memory,
    ...
    pub fn store_half_word(
        ...
        builder.write(
            ...
            addr1,
            byte1,
            Some("store half-word (LE)".to_string()),
        )
    }
    ...
    pub fn store_double_word(
        ...
        builder.write(
            ...
            a7,
            byte7,
            Some("store double-word (LE)".to_string()),
        )
    }
    ...
    pub fn check_access_alignment(
        ...
        let offset = builder.constd(sorts.sid_machine_word, (width - 1) as u64, None);

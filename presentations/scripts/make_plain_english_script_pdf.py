"""
Plain-English speaker script. No 'argv', no 'argc' — replaced with
'argument values' and 'argument counter'. Designed for listeners,
not readers.
"""
...
OUT = r"C:\Users\jasko\Programming\Rust\Project01\Symbolic_Arguments_Speaker_Script_PlainEnglish.pdf"
...
doc = SimpleDocTemplate(
    ...
    title="Symbolic Arguments — Speaker Script (plain English)",
)
...
story = []
story.append(Paragraph("Symbolic Arguments — plain English script", H_TITLE))
story.append(Paragraph("Same content, all abbreviations spoken in full. "
                       "argv → argument values. argc → argument counter.",
                       H_SUB))
...
story.extend(slide(2, "What 'symbolic arguments' actually means",
    ...
    "arguments make the argument values an open question. Each character of every symbolic "
    "argument is left free, and the model checker fills it in to find a bug. Concretely: "
    "Rotor lays out a normal argument layout on the stack, but instead of writing concrete "
    "characters, it writes 'unknown bytes' that the model checker gets to choose. The "
    "program reads its arguments exactly as it would in a real operating system — it doesn't "
    "know anything is symbolic.",
...
story.extend(slide(3, "Three command-line flags",
    "Three flags control the feature. The first one — the <b>symbolic-arguments</b> flag — "
    "turns it on. Without it, the program runs as if invoked with no arguments. The second "
    "— the <b>count</b> flag — says how many symbolic arguments to provide. The program name "
    "is always the literal string 'prog', so the total count the program sees is N plus one. "
    "The third — the <b>max-length</b> flag — says how many bytes of each argument are free. "
    "Each string is K plus one bytes counting the null terminator. Once these are parsed, "
    "the main function packs them into a configuration struct, and the command-line layer "
    "is done with symbolic arguments.",
    cue="The slide shows the literal flag names with dashes. You don't have to read the dashes — describe each flag in words."
))
...
story.extend(slide(5, "Four hooks in the core setup",
    ...
    "<b>First</b>: if the symbolic-arguments flag is on, we call the new function and use "
    "the stack pointer it returns; otherwise we use a default. <b>Second</b>: we write that "
    "stack pointer value into the stack pointer register. The program enters its main "
    "function with its stack pointer already pointing at the argument layout we built. "
    "<b>Third</b>: we write the argument counter into the first argument register — that's "
    "the Linux startup convention. <b>Fourth</b>: we make the stack value built by the new "
    "function the stack segment's starting value. After this line, when the program reads "
    "its arguments, the layout we built is what it sees."
))
...
story.extend(slide(7, "Phase one — compute the layout",
    ...
    "the bottom, the argument counter itself, with the stack pointer pointing right at "
    "it. The right column shows the variables we compute: the size of each region, where "
...
story.extend(slide(8, "Phase two — create the stack",
    ...
    "outside the argument area stay free, so the program may not assume any particular "
...
story.extend(slide(10, "Phase four — write the symbolic arguments  ★",
    ...
    "value — one line of code does it. That single line is the whole mechanism. The "
    "number of free bytes is exactly the count of symbolic arguments times the maximum "
    "length of each one. Every other byte on the stack is concrete; only these are free. "
    "The null terminator at the end of each string stays concrete — that way the C "
    "string convention is preserved, so functions like string-length still work.",
...
story.extend(slide(11, "Phase five — write the pointer array",
    ...
    "at the end — the standard Unix convention requires the array to be NULL-terminated. "
    "So when the program looks up the first argument, it dereferences a real address "
    "that points at the symbolic string we built in phase four."
))

story.extend(slide(12, "Phase six — write the argument counter",
    "Phase six writes the argument counter — the integer — at the address the stack "
    ...
    "argument counter is now in two places — on the stack here, and in the first "
    "argument register from earlier. The startup code can find it either way; both hold "
    "the same value."
))
...
story.extend(slide(13, "What the running program actually sees",
    ...
    "the first argument. <b>One</b>: it loads the pointer to the arguments from the "
    "stack — returns the address we wrote in phase five, a concrete address. "
    "<b>Two</b>: it loads the first argument's string address — returns the address of "
    "the first symbolic string. <b>Three</b>: it loads the first character of that "
    "string — returns the free eight-bit value we created in phase four. <b>Four</b>: "
    "it branches on that value. The comparison result is itself free; both sides of the "
    "branch enter the model. <b>Five</b>: the model checker picks values for every "
    "symbolic byte that satisfy whatever chain of branches leads to a bug. "
    "<b>Crucially</b> — nothing in the memory module, the register file, the decoder, "
    "or the kernel knows about symbolic arguments. They all just do their normal job.",
...
story.extend(slide(14, "End to end on a real benchmark",
    ...
    "to a RISC-V binary. <b>Step two</b>: Rotor, with the three symbolic-arguments "
    "flags, produces the BTOR2 model. The core setup is called once. The new function "
    "lays out arguments with two values of eight free bytes each. The stack pointer, "
    "the argument register, and the stack segment all get wired up. <b>Step three</b>: "
    "the model checker, given up to two hundred steps, finds the assignment — first "
    "character of the first argument equals 'X', first character of the second argument "
    "equals 'Y'. The program reaches return one; exit status is nonzero; the bad-exit "
    "property triggers. The witness trace contains those exact bytes. The visualizer "
    "can then load the model and the witness side by side and step through it.",
    cue="Concrete numbers — the values 'X' and 'Y' — are memorable. Say them clearly. This is the proof-it-works slide."
))

story.extend(slide(15, "Recap — every function on the path",
    "Every function on the path. <b>Command line</b>: the main function parses the "
    "flags and builds the configuration. <b>Pipeline</b>: the model generator loads "
    "the binary and calls the core setup. <b>Core setup</b>: branches on the flag, "
    "calls the new function, writes the stack pointer and the argument counter into "
    "the register file, attaches the stack value. <b>Argument layout</b>: the new "
    "function — six phases, all in one file. <b>Runtime reads</b>: ordinary byte "

...
% =====================================================================
% ART DECK 06 -- PEN & INK / ENGRAVING.
% Fine-art sketchbook: cross-hatching, stippling, confident contour
% lines, ink on warm paper. Engraved plates, EB Garamond lettering.
% Two pdflatex passes (overlay).
% =====================================================================
...
% full-bleed plate: (0,0)=SW .. (16,9)=NE
\newcommand{\art}[1]{%
  \begin{frame}[plain]
...
% ---- hatching: rake fine parallel lines (USE INSIDE a \clip scope) ---------
% {angle}{spacing cm}{linewidth}{color}
\newcommand{\hatch}[4]{%
...
% stipple inside a rectangle: {x0}{y0}{x1}{y1}{count}
\newcommand{\stipple}[5]{%
  \foreach \si in {1,...,#5}{
...
% engraved sphere: {(c)}{r}{hatch spacing}
\newcommand{\orb}[3]{%
  \begin{scope}
    \clip #1 circle (#2);
    \hatch{30}{#3}{0.4pt}{ink}
    \hatch{-35}{#3}{0.4pt}{ink}
    % deepen the lower-right crescent
    \begin{scope}\clip #1 ++(0.18,-0.18) circle (#2);
       \hatch{75}{#3}{0.4pt}{ink}\end{scope}
  \end{scope}
  % specular highlight (bare paper)
  \fill[paper] ($#1+(-0.32*#2,0.34*#2)$) circle (0.16*#2);
  \draw[pen2] #1 circle (#2);}

% small rounded rock with bottom shadow: {(c)}{rx}{ry}
\newcommand{\rock}[3]{%
  \begin{scope}
    \clip[shift={#1}] (0,0) ellipse (#2 and #3);
    \clip[shift={#1}] (-#2,-#3) rectangle (#2,{-0.12*#3});
    \hatch{25}{0.085}{0.4pt}{ink}
    \hatch{-30}{0.11}{0.4pt}{ink}
  \end{scope}
...
\art{
  ...
  % a few small firs on the ridge
  \pine{(11.9,5.3)}{0.5}\pine{(12.7,5.0)}{0.42}\pine{(13.5,4.9)}{0.46}
  % foreground left bank, grass hatch
  \begin{scope}
...
% =====================================================================
% 2. THE VAST FIELD  (2^64)
% =====================================================================
\art{
  \platetitle{The Haystack}
  % a great stippled expanse
  \begin{scope}
    \clip (1.6,1.6) rectangle (14.4,6.6);
    \stipple{1.6}{1.6}{14.4}{6.6}{2300}
  \end{scope}
  \draw[pen] (1.6,1.6) rectangle (14.4,6.6);
  % the one input: a bare ring with rays
  \fill[paper] (10.4,4.6) circle (0.55);
  \draw[pen2] (10.4,4.6) circle (0.32);
  \foreach \a in {0,30,...,330}{\draw[penf] (10.4,4.6)+(\a:0.62) -- +(\a:0.92);}
  \node[ink] at (10.4,3.4) {\scriptsize the one that crashes};
...
\art{
  ...
  \foreach \a in {0,15,...,345}{
     \pgfmathsetmacro{\rl}{1.0+mod(\a,30)*0.0+0.6*(1-mod(\a,30)/30)}
     \draw[penf] (8,5.6)+(\a:0.85) -- +(\a:{1.0+0.7*abs(sin(\a*3))});}
  % falling light onto the field (long rays)
  \foreach \x in {2.4,3.2,...,13.6}{\draw[penf, inkl] (8,5.6) -- (\x,3.0);}
  \platecap{can it reach a bad state, for \emph{any} input, within $k$ steps? --- one query covers them all}
}

% =====================================================================
% 4. THE APPARATUS  (pipeline)
% =====================================================================
\art{
  \platetitle{The Apparatus}
  \foreach \x/\lbl in {2.6/binary, 6.4/model, 10.2/solver, 13.6/picture}{
     % a drum / vessel
     ...
     \node[ink] at (\x,4.0) {\small\scshape \lbl};}
...
% =====================================================================
% 5. THE MECHANISM  (what a model is: a 24-tooth wheel)
% =====================================================================
\art{
...
\art{
  ...
  % tangled ball, dense scribble cross-hatch
...
% =====================================================================
% 7. SPEED  (two hourglasses)
% =====================================================================
\art{
  \platetitle{Step the Second --- Swiftness}
  % hourglass macro inline x, label, fill-fraction(top)
...
\art{
  ...
  % ledger column with endless tally
...
\art{
  ...
  \orb{(3.4,5.0)}{1.25}{0.10}
  \orb{(8.0,5.0)}{1.25}{0.15}
  \orb{(12.6,5.0)}{1.25}{0.26}
...
% =====================================================================
% 10. THE BACK-PORT  (meshing gears)
% =====================================================================
\art{
...
% =====================================================================
% 11. SYMBOLIC ARGUMENTS  (an opened lock)
% =====================================================================
\art{
  \platetitle{The Door Unsealed}
  % a grand padlock, shackle swung open
  ...
  % two small closed locks (the frozen past), faint
...
% =====================================================================
% 12. THE BALANCE  (validation)  -- centerpiece
% =====================================================================
\art{
  ...
  \begin{scope}\clip (\cx-1.0,1.7) rectangle (\cx+1.0,2.0);\hatch{30}{0.08}{0.4pt}{ink}\end{scope}
  % fulcrum + level beam
  \draw[pen2] (\cx,6.3) -- (\cx-0.28,6.0) -- (\cx+0.28,6.0) -- cycle;
  \draw[pen2] (\cx-3.4,6.25) -- (\cx+3.4,6.25);
  \draw[pen2] (\cx,6.25) circle (0.08);
  % chains + pans (both level)
  \foreach \px in {-3.4,3.4}{
     ...
     \begin{scope}\clip (\cx+\px-0.95,4.5) .. controls (\cx+\px-0.6,3.8) and (\cx+\px+0.6,3.8) .. (\cx+\px+0.95,4.5) -- cycle;
        \hatch{-25}{0.1}{0.4pt}{inkl}\end{scope}}
  \node[ink] at (\cx-3.4,3.5) {\scshape C rotor};
  \node[ink] at (\cx+3.4,3.5) {\scshape rust rotor};
  \node[ink] at (\cx,4.9) {\scshape\fontsize{30}{32}\selectfont 36 / 36};
...
\art{
  ...
  % bad node: a starred ring
  \foreach \a in {0,45,...,315}{\draw[penf] (g0)+(\a:0.5) -- +(\a:0.74);}
  \fill[paper] (g0) circle (0.42);\draw[pen2] (g0) circle (0.42);
  \node[ink] at (g0) {\scriptsize\scshape bad};
  \foreach \g in {g1,g2,g3,g4}{\fill[paper] (\g) circle (0.34);
...
\art{
  ...
  \draw[penf] (0.95,0.95) rectangle (15.05,8.05);
  % clouds
  \foreach \cx/\cy/\s in {3.4/7.2/1, 11.8/7.3/0.8}{
  ...
  % the peak, hatched on the shadow side
  \begin{scope}
    \clip (4.6,3.0) -- (8.0,7.4) -- (8.0,3.0) -- cycle;
    \hatch{60}{0.1}{0.4pt}{ink}
  \end{scope}
  \draw[pen2] (3.0,3.0) -- (8.0,7.4) -- (12.6,3.0);
  \draw[pen] (8.0,7.4) -- (6.6,5.0) -- (7.3,4.2) -- (6.2,3.3);
  % flag at the summit
  \draw[pen2] (8.0,7.4) -- (8.0,8.0);
  \draw[pen2] (8.0,8.0) -- (8.55,7.85) -- (8.0,7.7);
  \node[ink] at (8,2.15) {\scshape\fontsize{18}{20}\selectfont proving it still tells the truth};
  \node[ink] at (8,1.55) {\itshape\footnotesize was the whole climb. \quad thank you.};

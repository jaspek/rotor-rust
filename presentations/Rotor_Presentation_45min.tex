...
\begin{frame}{The Selfie Challenge: ``Just Model All of Selfie''}
  Input: pre-compiled selfie.m --- \textbf{43\,406 instructions};
  both rotors timed on the binary only (re-measured 2026-06-10).
  \vspace{0.1cm}
  \begin{center}
  \begin{tabular}{lrrr}
    \toprule
    ...
    \midrule
    model generation time & 139\,s & \textbf{$\sim$0.1\,s} &
      $\sim$1\,400$\times$\\
    peak memory           & 428\,MB & \textbf{20\,MB} &
      $\sim$21$\times$\\
    formula lines created & 3\,165\,611 & \textbf{$\sim$111\,k} &
      28$\times$\\
...
% ---------------------------------------------------------
\begin{frame}{Results: 18 / 18 --- Same Property, Same Step, Every Benchmark}
  \begin{center}
  \scriptsize
  \begin{tabular}{lllp{4.6cm}c}
    \toprule
    Benchmark & both report & $k$ & Benchmark (both report @ $k$) & \\
    \midrule
    division-by-zero & division-by-zero & 76 &
      simple-assignment @ 96 & \cellcolor{green!20}\checkmark\\
    invalid-memory-access & store-invalid-addr & 79 &
      simple-decreasing-loop @ 99 & \cellcolor{green!20}\checkmark\\
    memory-access-fail & load-seg-fault & 66 &
      simple-if-else @ 108 & \cellcolor{green!20}\checkmark\\
    nested-if-else & bad-exit-code & 100 &
      simple-if-else-reverse @ 108 & \cellcolor{green!20}\checkmark\\
    nested-if-else-rev & bad-exit-code & 103 &
      simple-if-without-else @ 101 & \cellcolor{green!20}\checkmark\\
    recursive-ackermann & bad-exit-code & 152 &
      simple-increasing-loop @ 93 & \cellcolor{green!20}\checkmark\\
    recursive-factorial & bad-exit-code & 119 &
      three-level-nested-loop @ 103 & \cellcolor{green!20}\checkmark\\
    recursive-fibonacci & bad-exit-code & 118 &
      two-level-nested-loop @ 99 & \cellcolor{green!20}\checkmark\\
    nested-recursion-fail & UNSAT & 1500 &
      return-from-loop: UNSAT @ 1500 & \cellcolor{green!20}\checkmark\\
    \bottomrule
  \end{tabular}
  \end{center}
  \vspace{0.1cm}
  \begin{block}{Final score: 18 / 18 benchmarks --- zero divergences}
    Five property types; recursion matched step-exactly. Bonus finding: the
    one apparent divergence was a bug in our own \emph{measurement harness}
    (a crashed solver container recorded as UNSAT) --- caught because one
    anomaly against 17 matches demanded investigation.
  \end{block}
\end{frame}

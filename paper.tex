...
% Keep each listing's frame in one piece: wrapping the environment in a
% minipage forbids column/page breaks inside the box, which would
% otherwise strand part of the frame at the top of the next column.
\usepackage{etoolbox}
...
\email{jasmin.begic@stud.plus.ac.at}
...
\begin{itemize}
  ...
  \item A counter-based performance analysis, following classical
        ...
        $\sim$93$\times$ faster with byte-identical output
...
24 properties are boolean formulas over that state --- for instance the
exit property reads
\begin{lstlisting}
40901 eq 1 20055 50 ; actual exit code ==
                    ; bad exit code?
\end{lstlisting}
\noindent while the division property (b7) guards the divisor of every
division instruction in the binary. Running \texttt{btormc} on the
model yields
\begin{lstlisting}
...
\midrule
\texttt{btor2/builder} & node interning (hash-consing), ids \\
...
\texttt{machine/memory, segm.} & segment arrays, bounds logic \\
...
\texttt{model/properties} & the 24 bad states, reference order \\
...
\begin{figure}[t]
\centering
\begin{tikzpicture}[yscale=0.52, xscale=0.62, font=\footnotesize]
  ...
  \draw[-{Stealth}] (-0.75,0.5) -- (-0.08,0.5);
  \node[anchor=east] at (-0.8,0.5) {\texttt{SP}};
  \node[anchor=east, black!60] at (9.5,4.4) {\scriptsize top of memory, $2^{32}$};
\end{tikzpicture}
...
argument count is fixed at generation time by the
\texttt{--num-symbolic-\allowbreak args} flag; the solver explores
argument \emph{content}, not count. \emph{(ii)}~Each argument occupies a
...
inspects. The relevant witness lines are literal enough to quote:
\begin{lstlisting}
sat
b23
#0
; ... 25 initial-memory assignments ...
...
hash-consed, byte-identical & 1.5\,s & 138{,}820 & \texttt{diff}, all outputs \\
hash-consed, dedup-everywhere & 1.5\,s & 99{,}k & 36/36 differential \\
naive dedup-everything & --- & --- & bad state at $k{=}0$ \\
\bottomrule
\end{tabular}
\end{table}

The byte-identical C rotor generates selfie's model in \textbf{1.5\,s}
instead of \textbf{139\,s} ($\sim$93$\times$), producing output that
...
structure, with the language held fixed. (All back-port timings here
were re-measured in the same containerized environment as
Table~\ref{tab:perf}; the companion repository additionally reports an
independent measurement on a different machine and selfie revision,
with the same two-orders-of-magnitude result.) The dedup-everywhere
variant
...
back into the reference C: there it is $\sim$93$\times$ faster with
...
engineering were assisted by AI tooling (Anthropic's Claude with the
versions Opus 4.7, 4.8 and Fable), following the course's guidance;
all measurements were produced by, and all claims are checkable
against, the committed harness and artifacts.
\end{acks}
